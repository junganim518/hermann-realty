...
[제목]
- 글 맨 앞에 제목 3개를 추천해주세요
- 각 제목은 SEO 키워드(지역명, 매물종류, 핵심 특징) 포함
- 클릭하고 싶게 만드는 후킹 문구로
- 이모지 1-2개 활용
- 형식:
  📝 제목 추천
...
[사진 자리]
- 본문 중간중간에 사진 들어갈 자리를 [사진 N] 형식으로 표시
- 외부 사진은 없으므로 내부 사진만 4-5장 배치
- 각 사진 자리 아래에 작은 추천 멘트 1줄
- 형식:
  [사진 1]
  추천 멘트: ○○○

[홈페이지 링크 안내 문구]
- 본문 마지막(해시태그 직전)에 헤르만부동산 홈페이지 매물 상세 페이지 안내 문구 포함
- 매물번호를 명시
- 형식 예시:
  💻 더 자세한 정보가 궁금하신가요?
  해당 매물의 더 많은 실사진과 상세 정보는 헤르만부동산 공식 홈페이지에서 확인하실 수 있습니다.
  👉 [매물번호 ${p.property_number ?? ''} 상세 정보 페이지 바로가기]
                    ...
                    <div className="admin-prop-info">
                      ...
                      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', fontSize: '13px', marginBottom: p.admin_memo ? '4px' : '0' }}>
                        ...
                        {p.transaction_type === '매매' && (p.current_deposit != null || p.current_rent != null) && (
                          <span style={{ color: '#555' }}>기보 {[p.current_deposit != null ? formatPrice(p.current_deposit) : null, p.current_rent != null ? formatPrice(p.current_rent) : null].filter(Boolean).join('/')}</span>
                        )}
